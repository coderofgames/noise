...
void NoiseMapBuilderCubeFace::Build ()
{
  ...
  // Create the plane model.
	...
	CUBE_PLANE curr_plane = (CUBE_PLANE)m_dir;

	int x = 0, y = 0;
	int percentage_done = 0;

	for( x = 0; x < m_destHeight ; x++ )
	{
		...
		for( y = 0; y < m_destWidth; y++ )
		{
			...
			// the target pixel occupies an integer location
			// in space on the square grid of dimensions m_destWidth*m_destWidth 
			// at a distance of m_destWidth/2 from the origin for a 90 degree cubemap
			// projection. This vector is later normalized into a unit sphere
			...
			float curValue = sphereModel.GetModule().GetValue (vdir.x, vdir.y, vdir.z);
...
*/
			...
			* (MAX_ELEV - MIN_ELEV)) + MIN_ELEV;
			...
			/*utils::WriterD3DTexture textureWriter;
			textureWriter.SetDestFilename( bmp_name );
			textureWriter.SetSourceImage( destImage01 );
...
//			utils::WriterD3DTexture textureWriter;
		/*	textureWriter.SetDestFilename( diff_name );
			textureWriter.SetSourceImage( destImage );
			textureWriter.WriteDestFile ();*/
			...
			// This is taken out for speed, until I start using the surface map
			...
			//utils::WriterD3DTexture textureWriter;
			/*textureWriter.SetDestFilename( surf_name );
			textureWriter.SetSourceImage( destImage );
			textureWriter.WriteDestFile ();*/
			...
			//utils::WriterD3DTexture textureWriter;
			/*textureWriter.SetDestFilename( spec_name );
			textureWriter.SetSourceImage( destImage );
			textureWriter.WriteDestFile ();*/
			...
			//utils::WriterD3DTexture textureWriter;
			/*textureWriter.SetDestFilename( normal_name );
			textureWriter.SetSourceImage( destImage );
			textureWriter.WriteDestFile ();*/
#endif
		}
